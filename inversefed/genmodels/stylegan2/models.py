...
def load(fpath, map_location='cpu'):
    ...
    Returns:
        model (nn.Module): Model that inherits `_BaseModel`.
    """
    fpath = "/home/qian/project/gradient-inversion-main/"+fpath
    # print("---------fpath:", fpath)
    # if map_location is not None:
    #     map_location = torch.device(map_location)  # cpu
    return _deserialize(torch.load(fpath, map_location=torch.device('cpu')))
